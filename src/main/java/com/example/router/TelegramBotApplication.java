...
import com.example.router.model.BotState;
    ...
    private void executeCashedCommand(BotState botState, String parameter, String userName,Long chatId){
        switch (botState){
            ...
            case ADD_ROLE_OF_USER_FOR_CREATION_USER:{
                connectionToCommandService.insertUser(usernameForCreationCash.get(userName),"",parameter);
    ...
    private void executeCommand(String commandName, Long chatId, String userName){
        switch (commandName) {
            case "/start": {
                ...
                String responseFromRouter=connectionToCommandService.getRoleNameByUserName(commandName);
                ...
                if(role.equals("admin")){
                    SendMessage response = new SendMessage(chatId, "Ваша роль admin\n" +
                            ...
                            "/add_group /delete_group /rename_group /delete_users_from_group" +
            ...
            case "/add_users_to_group": {
                ...
                SendMessage response = new SendMessage(chatId, "Введите название группы для добавления пользователей");
                this.execute(response);
                break;
            }
            case "/delete_users_from_group": {
                botStateCash.put(userName,BotState.DELETE_USER_FROM_GROUP);
                SendMessage response = new SendMessage(chatId, "Введите название группы удаления пользователей");
                this.execute(response);
                break;
            }
            case "/del_user":{
